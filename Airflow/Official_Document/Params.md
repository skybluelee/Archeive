...
|-|-|-|-|
|`string|텍스트를 수정하기 위해 한 줄의 텍스트 박스를 생성한다.|- `minLength(최소 길이):` 텍스트의 최소 길이 <br>- **maxLength(최대 길이):** 텍스트의 최대 길이 <br>- **format="date"(형식="date"):** 달력 팝업이 있는 날짜 선택기 생성 <br>- **format="datetime"(형식="datetime"):** 달력 팝업이 있는 날짜 및 시간 선택기 생성 <br>- **format="time"(형식="time"):** 시간 선택기 생성 <br>- **enum=["a", "b", "c"](열거=["a", "b", "c"]):** 스칼라 값에 대한 드롭다운 선택 목록 생성. JSON 유효성 검사에 따르면 값이 선택되어야 하거나 필드를 명시적으로 선택 가능하게 표시해야 합니다. Enum에 대한 자세한 내용은 JSON Schema 설명 내부의 세부 정보를 참조하십시오. <br>- **values_display={"a": "Alpha", "b": "Beta"}(values_display={"a": "Alpha", "b": "Beta"}):** enum을 통해 생성된 선택 드롭다운에는 데이터 값과 표시 레이블을 매핑하는 dict와 함께 values_display 속성을 추가할 수 있습니다. <br>- **examples=["One", "Two", "Three"](examples=["One", "Two", "Three"]):** 사용자에게 특정 값 제안을 제시하려면(위의 고정된 enum으로 사용자를 제한하지 않음) examples를 사용할 수 있습니다. 이는 항목 목록입니다. <br>또한, 백엔드에서 DAG 트리거 전에 확인되는 추가 JSON Schema 문자열 유형 유효성 옵션에 대한 자세한 내용은 JSON Schema 문서를 참조하십시오.| ㅇㅇ|
***
